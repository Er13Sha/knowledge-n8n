...
export function useKnowledgeBase(notify: Notify) {
    ...
    const feedbackRating = ref<'positive' | 'negative' | null>(null);
    ...
    async function sendFeedback(
        ...
        const interactionId = searchResult.value?.interaction_id;

        if (interactionId === undefined || isSubmittingFeedback.value) {
            return;
        }
        ...
        try {
            await knowledgeApi.feedback(interactionId, rating);
            feedbackRating.value = rating;
            notify('Спасибо за оценку ответа.');
        } catch (error) {
            notify(errorMessage(error), 'error');
        } finally {
    ...
    async function copySearchAnswer(): Promise<void> {
        ...
        const sources = result.sources.length
            ? `\n\nИсточники:\n${result.sources.map((source) => `[${source.number}] ${source.document_name}, стр. ${source.page}`).join('\n')}`
            : '';

        try {
            await navigator.clipboard.writeText(`${result.answer}${sources}`);
            notify('Ответ и источники скопированы.');
        } catch (error) {
            notify(errorMessage(error), 'error');
        }
    }
