...
use App\Http\Controllers\Admin\KnowledgeSearchQualityController;
use App\Http\Controllers\Api\AuthController;
use App\Http\Controllers\Internal\RagController;
use App\Http\Controllers\Extraction\DocumentExtractionController;
...
use App\Http\Controllers\Knowledge\KnowledgeSearchFeedbackController;
...
Route::middleware(['web', 'auth'])->prefix('knowledge')->name('api.knowledge.')->group(function () {
    ...
    Route::post('search/{interaction}/feedback', KnowledgeSearchFeedbackController::class)->name('search.feedback');
});
...
Route::middleware(['web', 'auth', EnsureSuperAdmin::class])->prefix('admin')->name('api.admin.')->group(function () {
    ...
    Route::get('knowledge/search-quality', KnowledgeSearchQualityController::class)->name('knowledge.search-quality');
});

Route::middleware(['web', 'auth'])->prefix('extractions')->name('api.extractions.')->group(function (): void {
    Route::get('/', [DocumentExtractionController::class, 'index'])->name('index');
    Route::post('/', [DocumentExtractionController::class, 'store'])->name('store');
    Route::get('{extraction}', [DocumentExtractionController::class, 'show'])->name('show');
    Route::post('{extraction}/retry', [DocumentExtractionController::class, 'retry'])->name('retry');
    Route::delete('{extraction}', [DocumentExtractionController::class, 'destroy'])->name('destroy');
    Route::get('{extraction}/download', [DocumentExtractionController::class, 'download'])->name('download');
    Route::get('{extraction}/download-json', [DocumentExtractionController::class, 'downloadJson'])->name('download-json');
});
